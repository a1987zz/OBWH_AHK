/*
    ...
    **Notes:
    ...
    Internal methods: (some may be useful when directly called)
        getAddressFromOffsets() ; Это вернет окончательный адрес памяти указателя. Это полезно, если указанный адрес изменяется только при запуске или изменении карты/уровня, и вы хотите устранить накладные расходы, связанные с указателями.
        ...
        openProcess()
        closeHandle() 
    Useful properties:  (Не изменяйте значения этих свойств - они устанавливаются автоматически)
